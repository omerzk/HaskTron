...
update input game = if state game then GameState {state = (fst (input !! 2) == 0), players = players'} else  if snd (input !! 2)  == 0 then GameState {state = (1,1) `elem` input, players = players game} else defaultGame -- redundent and ugly
...
render game (w, h)  =
  if state game then centeredCollage w h (back:(concatMap playerForm (players game))) else centeredCollage w h pauseScreen  -- ++ others-- ++ $map shotForm survivors --(\pState -> playerForm pState) survivors--do i even need the lambda?
    where back1 = gradient grad $ square 750
          ...
          pauseScreen = [(gradient (linear (0,0) (100,100) [(0, maroon),(1, teal)])  $ ngon 8 200),move (0, -125) (filled red $ oval 50 30), txt]
...
-- TODO: this is crap, instead of saving every (x,y) save every nth pair and for the other use a last coordinate field, where in the form func chain the field to the coordinates -- THIS IS LIKELY TO ELIMINATE THE WIGGLING MOTIONS
